 ...
 * but this would lead to larger number of classes to manage so.. trying to do
 * one for all
 * 
 */
@Getter
...
public class ParticleServerSentEventSourceTask extends SourceTask {
	...
	public void start(Map<String, String> map) {
		 ...
		 * responsible for bootstrapping itself - but I also want the flexibility to
		 * inject
		 * other implementations/dependencies for testing hence I check first if context
		 * is not null (Running in Connect runtime).
		 * 
		 * This works so long as my business logic doesn't interact with the context
		 * class which
		 * I then need to test - at that point this approach will need to become more
		 * sophisticated
		 * 
		 * TODO:
		 * https://docs.confluent.io/platform/7.5/connect/kafka-connect-maven-plugin/site/kafka-connect-mojo.html
		 */

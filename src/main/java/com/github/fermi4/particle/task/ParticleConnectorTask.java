...
public final class ParticleConnectorTask {
	...
	public static Class<? extends Task> get(ParticleConnectorConfig config) {
		if(config.getConnectorMode() == PARTICLE_CONNECTOR_MODE_SSE) {
			return sseTask();			
		} else if (config.getConnectorMode() == PARTICLE_CONNECTOR_MODE_HEALTH) {
			throw new ConnectException(String.format("healthcheck mode not implemented yet"));
		}
		throw new ConnectException(String.format("Connector mode not recognized: [%s]... check configuration for [%s]", config.getAccessMode(), PARTICLE_CONNECTOR_MODE_CONFIG));
	}

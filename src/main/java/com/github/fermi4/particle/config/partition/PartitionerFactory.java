...
import com.github.fermi4.particle.client.ParticleClientFactory;
...
public class PartitionerFactory {
	...
	public static ParticleTaskConfigPartitioner get(ParticleConnectorConfig config) {
		if (config.getDiscover() != null) {
			...
			switch (config.getDiscover()) {
				...
				case ParticleConnectorConfig.DISCOVER_MODE_DISCOVER_ALL_DEVICES_IN_PRODUCT:
					return PartitionerFactory.getDevicesInProductPartitioner(client,
							config.getProductId());
				case ParticleConnectorConfig.DISCOVER_MODE_DISCOVER_ALL_DEVICES_IN_PRODUCT_AND_GROUPS:
					List<String> productGroups = List
							.of(config.getProductGroups().split(config.getDelimiter()));
					return PartitionerFactory.getDevicesInProductAndGroupsPartitioner(client,
							config.getProductId(), productGroups);
				default:
					throw new ConnectException("Discover mode not recognized ["
							+ config.getDiscover() + "] check "
							+ ParticleConnectorConfig.DISCOVER_MODE_CONFIG);
			}
		}
		if (config.getEventMode() != null) {
			switch (config.getEventMode()) {
				...
				default:
					throw new ConnectException("Event mode not recognized [" + config.getEventMode()
							+ "] check " + ParticleConnectorConfig.EVENT_MODE_CONFIG);
			}
		}
		throw new ConnectException("Error getting partitioner -- need to set either "
				+ ParticleConnectorConfig.EVENT_MODE_CONFIG + " or "
				+ ParticleConnectorConfig.DISCOVER_MODE_CONFIG);
	}
	...
	/**
	 * TODO: The below partitioners are based on discovering devices/product event
	 * streams to target by utilizing the particle API
	 * These should be moved out into a separate "discovery" interface instead of
	 * `ParticleTaskConfigPartitioner`
	 * but this works for now by just combining the concepts of discovery and
	 * partitioning
	 */

	public static ParticleTaskConfigPartitioner getDevicesInProductAndGroupsPartitioner(
			ParticleClient particleClient, String productIdOrSlug, List<String> productGroups) {
		return new DevicesInGroupsPartitioner(particleClient, productIdOrSlug, productGroups);
	}

	public static ParticleTaskConfigPartitioner getDevicesInProductPartitioner(ParticleClient particleClient,

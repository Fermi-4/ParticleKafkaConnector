...
public class ParticleEndpointSupplier {
	...
	public static HttpUrl get(ParticleConnectorConfig config) throws ConnectException {
		System.out.println(String.format("Checking arg [%s] against key: [%s]", config.getEventMode(), EVENT_MODE_ALL));
	    ...
	    switch (config.getEventMode()) {
	        ...
	        default:
	            throw new ConnectException(String.format("Endpoint supplier could not be determined! Event mode [%s] not recognized! Check configuration parameter [%s]", config.getEventMode(), ParticleConnectorConfig.EVENT_MODE_CONFIG));
	    }
	}

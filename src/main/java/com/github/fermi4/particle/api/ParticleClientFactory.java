...
public class ParticleClientFactory {
	public static ParticleClient get(ParticleConnectorConfig config) {
		...
		if (version == null) {
			throw new ConnectException("API version cannot be null when using ParticleClient -- check " + ParticleConnectorConfig.API_VERSION_CONFIG);
		}
		switch(version) {
			case ParticleConnectorConfig.PARTICLE_CONNECTOR_API_VERSION_1:
				return ParticleClientFactory.getV1ParticleClient(config);
			default:
				throw new ConnectException("API version not recognized for constructing ParticleClient -- check " + ParticleConnectorConfig.API_VERSION_CONFIG);
		}
	}
	...
	protected static OkHttpClient createOkHttpClientFromConfig(HttpClientConfig httpClientConfig) {
		...
		// if (httpClientConfig.getProxyHost() != null && !httpClientConfig.getProxyHost().isEmpty()) {
		// 	if (httpClientConfig.getProxyHost().toLowerCase() == ParticleConnectorConfig.HTTP_PROXY_HOST_CONFIG_NO_PROXY) {
		// 		httpClientBuilder.proxy(Proxy.NO_PROXY);
		// 	} else {
		// 		Proxy proxy = new Proxy(Proxy.Type.HTTP,new InetSocketAddress(httpClientConfig.getProxyHost(), httpClientConfig.getProxyPort()));
